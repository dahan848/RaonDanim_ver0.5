...
// 		alert(checkoutDate);
		...
		//숙소 제목 유효성검사
		if(motel_title == "" || motel_title.length < 3 || motel_title.length > 25){
			swal({
		...
		var priceCheck = /^[0-9]+$/;
			if(!priceCheck.test(motel_price) || motel_price > 10000){
				swal({
		               text:"가격을 확인해 주세요.",
		               icon:"warning",
		               buttons:[false,"확인"]
...
<%-- 								<input type="text" name="user_num" value="${user_num }"> --%>
							<div class="form_all" style="width: 90%; height: 90%;">
								...
								<h4>
									숙소의 요금을 설정해 주세요.(요금을 0원 설정 시 게스트가 무료로 이용 가능합니다.)<br>
								</h4>
